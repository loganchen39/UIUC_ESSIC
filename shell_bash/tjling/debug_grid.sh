...
writeoutarray()
{
	 ...
	 ## echo "dimstr=''" >> $subname
	  ...
	  echo $i | sed -e "s/.*:://g" > subnow
	     ...
	     echo " write(6,*) \"shape of grid%$j is \", shape(grid%$j) ">>$subname
	    ##  if echo $j | grep -iE "_BTXE|_BTXS|_BTYE|_BTYS|_BXE|_BXS|_BYE|_BYS" 
	    ##  then
	    ##  # _BTXE
	    ##  # _BTXS
	    ##  # _BTYE
	    ...
	    ##  # _BYE
	    ##  # _BYS
	    ##  echo " write(6,*) \"lbound of grid%$j is \", lbound(grid%$j) ">>$subname
	    ##  echo " write(6,*) \"ubound of grid%$j is \", ubound(grid%$j) ">>$subname
	    ##  fi
          done
...
then
   ...
   itimestep="itimestep"
   sed -n "35,2027p" module_domain_type.f90 > now
   #cp cplines now
else
   echo "Running $0 $1"
   chmod 700 $1
...
do
   ...
   if echo $i |  grep -v "^type"  > /dev/null
   then
	...
	echo $i | sed -e "s/.*:://g" > subnow
        ...
        for j in `cat subnow`
        do
	   echo "write(6,*) \"grid%$j=\",grid%$j" >> $subname
        done
     fi
   fi
done
